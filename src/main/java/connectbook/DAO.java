...
public class DAO {
	...
	public List getChatHeadsById(int id){
		List list_Messages;
		Query query = manager.createQuery("select u.name,u.dp,u.username,m.msg, m.msgTime from connectbook.Entity.Message m , connectbook.Entity.Users u where u.id = m.msgBy and m.msgTo = :id or u.id = m.msgTo and m.msgBy = :id");
		query.setParameter("id", id);
